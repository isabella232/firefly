...
func (am *assetManager) PrepareOperation(ctx context.Context, op *fftypes.Operation) (*fftypes.PreparedOperation, error) {
	...
	case fftypes.OpTypeTokenTransfer:
		...
		return opTransfer(op, pool.ProtocolID, transfer), nil
...
func (am *assetManager) RunOperation(ctx context.Context, op *fftypes.PreparedOperation) (complete bool, err error) {
	...
	case transferData:
		...
		case fftypes.TokenTransferTypeMint:
			return false, plugin.MintTokens(ctx, op.ID, data.PoolProtocolID, data.Transfer)
		case fftypes.TokenTransferTypeTransfer:
			return false, plugin.TransferTokens(ctx, op.ID, data.PoolProtocolID, data.Transfer)
		case fftypes.TokenTransferTypeBurn:
			return false, plugin.BurnTokens(ctx, op.ID, data.PoolProtocolID, data.Transfer)
		default:
...
func opTransfer(op *fftypes.Operation, poolProtocolID string, transfer *fftypes.TokenTransfer) *fftypes.PreparedOperation {
	return &fftypes.PreparedOperation{
		ID:   op.ID,
		Type: op.Type,
		Data: transferData{PoolProtocolID: poolProtocolID, Transfer: transfer},
	}
}
